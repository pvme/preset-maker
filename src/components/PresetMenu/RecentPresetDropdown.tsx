...
import { removeRecentPreset } from "../../storage/recent-presets";
...
export const RecentPresetDropdown = ({
  ...
  const [presetToRemove, setPresetToRemove] =
    useState<PresetSummary | null>(null);

  const recent = items;

  /* -------------------------------------------------------------------------- */
  /* Ensure Select value is always valid                                         */
  /* -------------------------------------------------------------------------- */

  const validSelected = useMemo(() => {
    if (!selected) return "";
    return recent.some((r) => r.presetId === selected) ? selected : "";
  }, [selected, recent]);
  ...
  }, [selected, validSelected, onSelect]);
          ...
          renderValue={(v) => {
            ...
            return (
              <Box display="flex" alignItems="center">
                ...
                <Box ml={match.source === "cloud" ? 1 : 0}>
                  {match.presetName.length > 50
                    ? `${match.presetName.slice(0, 47)}…`
                    : match.presetName}
                </Box>
              </Box>
            );
